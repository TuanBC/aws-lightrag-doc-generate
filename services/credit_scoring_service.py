...
class CreditScoringService:
    ...
    def extract_features(
        ...
        features["contract_interactions"] = (
        ...
        features["failed_tx_ratio"] = features["failed_transactions"] / max(
            1, features["total_transactions"]
        )
        ...
        if len(df) > 1:
            ...
            features["max_inactivity_days"] = (
                inactivity_periods.max() if not inactivity_periods.empty else 0
            )
        else:
            features["max_inactivity_days"] = 0

        # Weekday pattern
        features["most_active_weekday"] = (
            int(df["timeStamp"].dt.dayofweek.mode()[0]) if not df.empty else None
        )
        ...
        features["largest_incoming_tx"] = (
            wei_to_eth(incoming_txs["value"].max()) if not incoming_txs.empty else 0
        )
        features["largest_outgoing_tx"] = (
            wei_to_eth(outgoing_txs["value"].max()) if not outgoing_txs.empty else 0
        )
        ...
        if len(df) > 1:
            ...
            features["avg_time_between_tx_days"] = (
                time_diffs.mean() / 86400 if not time_diffs.empty else 0
            )
            features["std_time_between_tx_days"] = (
                time_diffs.std() / 86400 if not time_diffs.empty else 0
            )
        ...
        for label, cutoff in [("6m", months_ago_6), ("12m", months_ago_12)]:
            ...
            features[f"largest_tx_value_{label}"] = (
                float(wei_to_eth(dfx["value"].max()))
                if not dfx.empty and dfx["value"].notna().any()
                else 0.0
            )

            features[f"avg_tx_value_{label}"] = (
                float(wei_to_eth(dfx["value"].mean()))
                if not dfx.empty and dfx["value"].notna().any()
                else 0.0
            )

            features[f"failed_tx_count_{label}"] = (
                int((dfx["isError"] == 1).sum()) if not dfx.empty else 0
            )
            ...
            monthly_data.append(
                {
                    ...
                    "net_eth": wei_to_eth(recv_df["value"].sum())
                    - wei_to_eth(sent_df["value"].sum()),
                    ...
                    "avg_tx_value": wei_to_eth(month_df["value"].mean())
    ...
    def calculate_credit_score(
        self, features: Dict[str, Any], card_info: Optional[Dict[str, Any]] = None
    ) -> float:
        """
