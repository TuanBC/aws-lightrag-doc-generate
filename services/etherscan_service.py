...
class EtherscanService:
    ...
    async def fetch_card_info(self, address: str) -> Dict[str, Any]:
        ...
        async with aiohttp.ClientSession() as session:
            async with session.post(
                self.ETHERSCAN_CARDS_URL, headers=headers, json=json_data, timeout=30
            ) as resp:
                if resp.status != 200:
                    text = await resp.text()
                ...
                if h4_tag:
                    ...
                    for h4 in h4s:
                        ...
                        if match:
                            ...
                            break
                        if (
                            "no badges" in h4_text
                            or "no attestations" in h4_text
                            or "no poaps" in h4_text
                            or re.search(r"0/\d+", h4_text)
                        ):
                            value = "0"
                            break
    ...
    def _to_snake_case(s: str) -> str:
        ...
        replacements = {
            ...
            "Builder Score": "builder_score",
            "AML Risk Score": "aml_risk_score",
            "Reputation Score": "zscore_reputation_score",
